...
//		String filePath = "/Users/acacia99/works/search/data/";
			...
			for (int i = 0; i < listFile.length; i++) {
				...
				if(listFile[i].isFile() && listFile[i].getName().indexOf(".log") > -1){
					...
					while((strLine=br.readLine()) != null){
						...
						//Pattern p1 = Pattern.compile("GET\\s*(http|https|ftp)://[^\\s^\\.]+(\\.[^\\s^\\.]+)*\\s*HTTP");
						...
						Pattern p4 = Pattern.compile("GET\\s*(http|https|ftp)://[^\\s^\\.]+(.gsshop.com/jsp/dummylog.jsp\\?ab=A&lseq=386749)");
						Matcher m4 = p4.matcher(strLine);
						if(m4.find()){
							
							prdaCnt++;
						}
						Pattern p5 = Pattern.compile("GET\\s*(http|https|ftp)://[^\\s^\\.]+(.gsshop.com/jsp/dummylog.jsp\\?ab=B&lseq=386750)");
		...
		.append(args[0]).append("\t")
		.append(lseq386749Cnt).append("\t")
		.append(prdaCnt).append("\t")
		.append((float)(prdaCnt*100/(lseq386749Cnt*1.0))).append("\t")
		.append(lseq386750Cnt).append("\t")
		.append(prdbCnt).append("\t")
		.append((float)(prdbCnt*100/(lseq386750Cnt*1.0))).append("\t")
		.append("\n");
		...
		try {
			writer = new FileWriter("/wiselog/rawdata/filedata/" + args[0] + ".txt");
			writer.write(stb.toString());
			writer2 = new FileWriter("/wiselog/rawdata/filedata/wiselog_ab_data.txt", true);
			writer2.write(stb2.toString());
